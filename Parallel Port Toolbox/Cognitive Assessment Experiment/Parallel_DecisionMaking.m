%% Decision Making Task for Parallel Port Markering
...
pname = strcat('Parallel_DecMakBeh_',subject_number);
...
for block = 1:num_of_blocks
    ...
    for trial = 1:num_of_trials
        ...
        Screen('FillRect', win , square_colour(colour_side(2),:), [xmid+100 ymid-50 xmid+200 ymid+50]);
        DrawFormattedText(win,'+','center','center',[75 75 75]);
        parallel_flipandmark(parallel_port+20,win,marker+2,use_parallel); %Sends the current marker 
        ...
        DrawFormattedText(win,Feedback_Msg,'center','center',[255 255 255]);
        parallel_flipandmark(parallel_port,win,feedback_marker,use_parallel); %Sends the current marker 
        WaitSecs(1);
